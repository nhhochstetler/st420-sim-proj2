...
Additionally upon further investigating the data, we found several data issues with some columns where data varies greatly for the same country year over year. In many cases, it appears to be input error (i.e. missing a digit). We try to eliminate records that are out of ordinary by comparing to the average of the country for some columns. But there are some columns that are so messy that we deem beyond repair therefore excluded from our analysis.

Below are the plots showing we have discovered data issue where some values appear to be miss coded and miss a digit. We have come up with a method to exclude values that are way too off comparing to other years within the same country 
...
Check normality assumption through a QQ-Plot for each of the models. The top 4 are the selected models, while the bottom 3 are the base models we started with.
```{r}
par(mfrow= c(2, 3))

qqnorm(resid(le_full_add_model_sel))
qqline(resid(le_full_add_model_sel))

qqnorm(resid(le_add_model_sel))
qqline(resid(le_add_model_sel))

qqnorm(resid(le_inter_model_sel))
...
High level summary of assumption violation and p-Value of initially fitted model
...
Despite the high r.squared value overall, all the models failed BP test and Shapiro test which indiate that the homoscedasticity assumption and normality assumption are violated. In below section, we attempt to search for a model from all possible combinations of selected additive predictors that has fair r.sqaure and passes both assumption tests.
...
We searched through 255 combinations of models and only very few models that pass BP test and none seems to pass Shapiro test. So We proceed to pick one from the few models that pass the BP test.
